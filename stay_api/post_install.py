...
def stay_api_postinstall(cr, registry):
    with api.Environment.manage():
        ...
        model_datas = env['ir.model.data'].search([('module', '=', 'base'), ('model', '=', 'res.partner.title'), ('res_id', '!=', False), ('name', '!=', False)])
        unique_code = set()
        for model_data in model_datas:
            stay_code = model_data.name.split('_')[-1]
            if stay_code in unique_code:
                logger.warning('Skipping XMLID %s.%s because the suffix is not unique', model_data.module, model_data.name)
                continue
            unique_code.add(stay_code)
            title = env['res.partner.title'].browse(model_data.res_id)
            title.write({'stay_code': stay_code})
            logger.info('Wrote stay_code=%s on title %s ID %d', stay_code, title.display_name, title.id)
