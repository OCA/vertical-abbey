...
class StayStay(models.Model):
    ...
    partner_name = fields.Text(
        'Guest Name', required=True, tracking=True)
    guest_qty = fields.Integer(
        string='Guest Quantity', default=1, tracking=True)
    arrival_date = fields.Date(
        string='Arrival Date', required=True, tracking=True,
        index=True)
    arrival_time = fields.Selection([
        ('morning', 'Morning'),
        ('afternoon', 'Afternoon'),
        ('evening', 'Evening'),
        ], string='Arrival Time', required=True, tracking=True)
    arrival_datetime = fields.Datetime(
        compute='_compute_arrival_datetime', store=True,
        string='Arrival Date and Time')
    departure_date = fields.Date(
        string='Departure Date', required=True, tracking=True,
        index=True)
    departure_time = fields.Selection([
        ('morning', 'Morning'),
        ('afternoon', 'Afternoon'),
        ('evening', 'Evening'),
        ], string='Departure Time', required=True, tracking=True)
    departure_datetime = fields.Datetime(
        compute='_compute_departure_datetime', store=True,
        string='Departure Date and Time')
    room_id = fields.Many2one(
        'stay.room', string='Room', tracking=True, copy=False,
    ...
    group_id = fields.Many2one(
        'stay.group', string='Group', tracking=True, copy=False,
        domain="[('company_id', '=', company_id)]",
    ...
    user_id = fields.Many2one(
        related='group_id.user_id', store=True)
        ...
        for stay in self:
            if stay.arrival_date > stay.departure_date:
                raise ValidationError(_(
                    ...
                    'departure date (%s)!')
                    % (format_date(self.env, stay.arrival_date), format_date(self.env, stay.departure_date)))
            if stay.arrival_date == stay.departure_date:
            ...
            if qty > bed_qty:
                raise ValidationError(_(
                    ...
                    "%d guest on %s whereas room '%s' only has %d beds.") % (
                        self.display_name, guest_qty, qty,
...
class StayRoom(models.Model):
    ...
    sequence = fields.Integer(default=10)
    group_id = fields.Many2one(
        'stay.group', string='Group', check_company=True,
        domain="[('company_id', '=', company_id)]")
    user_id = fields.Many2one(
    ...
    def _stay_notify(self):
        ...
        for group in self.search([('notify_user_ids', '!=', False)]):
            ...
            if stays:
                ...
                email_from = self.company_id and self.company_id.email or self.env.user.email
...
class StayLine(models.Model):
    ...
    refectory_id = fields.Many2one(
        'stay.refectory', string='Refectory', check_company=True,
    ...
    group_id = fields.Many2one(
        related='room_id.group_id', store=True)
    user_id = fields.Many2one(
        related='room_id.group_id.user_id', store=True)
