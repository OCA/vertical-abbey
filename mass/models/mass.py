...
class MassRequest(models.Model):
    ...
    celebrant_id = fields.Many2one(
        'res.partner', string='Celebrant', domain=[('celebrant', '=', 'internal')],
    ...
    product_id = fields.Many2one(
        'product.product', string="Mass Product", check_company=True,
        domain="['|', ('company_id', '=', False), ('company_id', '=', company_id), ('mass', '=', True)]",
        required=True, readonly=True, ondelete='restrict',
        states={'waiting': [('readonly', False)]})
    type_id = fields.Many2one(
        related='product_id.mass_request_type_id', store=True,
    ...
    stock_account_id = fields.Many2one(
        'account.account', string='Stock Account', check_company=True,
        required=True,
        ...
        for request in self:
            if request.state != 'waiting':
                raise UserError(_(
                    ...
                    "it is not in Waiting state.") % request.display_name)
        return super().unlink()
...
class MassLine(models.Model):
    ...
    request_id = fields.Many2one(
        'mass.request', string='Mass Request', ondelete='cascade',
    ...
    request_date = fields.Date(
        ...
        string="Mass Request Date")
    product_id = fields.Many2one(
        related='request_id.product_id', store=True)
    type_id = fields.Many2one(
            ...
            for mass in self:
                if mass.state == 'done':
                    raise UserError(_(
                        ...
                        'it is in Done state.')
                        % (format_date(self.env, mass.date), mass.partner_id.display_name))
                if mass.type_id.uninterrupted and mass.date < last_date:
                    raise UserError(_(
                        ...
                        % (format_date(self.env, mass.date), mass.partner_id.display_name, mass.type_id.name))
        return super().unlink()
...
class MassRequestTransfer(models.Model):
    ...
    def name_get(self):
        ...
        res = []
        for trf in self:
            name = '%s %s (%s)' % (trf.celebrant_id.display_name, format_date(self.env, trf.transfer_date), state2label.get(trf.state))
        ...
        for trf in self:
            if trf.state == 'done':
                raise UserError(_(
                    ...
                    % (format_date(self.env, trf.transfer_date), trf.celebrant_id.display_name))
        return super().unlink()
