...
class DonationDonation(models.Model):
    ...
    def _compute_mass_request_count(self):
        ...
        mapped_data = dict(
            [(x['donation_id'][0], x['donation_id_count']) for x in rg_res])
                ...
                for mass_request in mass_requests:
                    if mass_request.state != 'waiting':
                        raise UserError(_(
                            ...
                            '%s state.')
                            % (donation.display_name, mass_request._fields['state'].convert_to_export(mass_request.state, mass_request)))
                self.message_post(body=_('%d related mass request(s) in waiting state '
